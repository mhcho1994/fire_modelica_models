...
model ChassisMassProperties "Independent full-tensor mass composition and aggregate-mode checks"
  FIRE_Modelica.Physical.Mechanical.Chassis.ChassisAssembly assembled(
    useAssembledMass = true,
    core(mass = 2, r_C = {1, -2, 3}, inertia = diagonal({2, 3, 4}), componentId = "frame"),
    nArms = 1,
    arms = {FIRE_Modelica.Physical.Mechanical.Chassis.Arms.RigidArm(
  ...
  FIRE_Modelica.Physical.Mechanical.Chassis.ChassisAssembly aggregateOnly(
    useAssembledMass = false,
    aggregate(mass = 7, r_C = {4, 5, 6}, inertia = diagonal({1, 2, 3})),
    core(mass = 900), nPayloads = 1,
    payloads = {FIRE_Modelica.Physical.Mechanical.Chassis.Payloads.FixedPayload(mass = 800)});
  FIRE_Modelica.Physical.Mechanical.Chassis.ChassisAssembly coreOnly(
