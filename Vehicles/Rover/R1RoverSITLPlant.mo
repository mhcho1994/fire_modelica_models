...
model R1RoverSITLPlant
  ...
  parameter Real maxWheelTurn = 35 * fire_modelica_models.Utilities.Constants.d2r
    "Maximum steering wheel angle [rad]";
  parameter Real skidTurnRate = 140 * fire_modelica_models.Utilities.Constants.d2r
    "Skid-steer yaw rate at full steering [rad/s]";
  parameter Real vectoredTurnRateMax = 120 * fire_modelica_models.Utilities.Constants.d2r
...
equation
  ...
  turnDiameter =
    if abs(steeringLimited) < fire_modelica_models.Utilities.Constants.eps then 0
  ...
  yawRate =
    ...
    elseif abs(steeringLimited) < fire_modelica_models.Utilities.Constants.eps or abs(speed) < fire_modelica_models.Utilities.Constants.eps then 0
    else 2 * fire_modelica_models.Utilities.Constants.pi * speed / (fire_modelica_models.Utilities.Constants.pi * turnDiameter);

  longitudinalAccel = maxAccel * (targetSpeed - speed) / max(maxSpeed, fire_modelica_models.Utilities.Constants.eps);
  ...
  accelerationBody = {longitudinalAccel, lateralAccel, -fire_modelica_models.Utilities.Constants.g};
